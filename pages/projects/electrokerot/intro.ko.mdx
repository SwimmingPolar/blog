...
제공하는 그룹을 개벌적으로 선택해 부품을 구매할 경우, 최대 10% ~ 15%의 가격차이가 발생한다. 평균적인 데스트탑
...
개발 범위가 지속적으로 넓어지는 경우가 많았기 때문에, 처음부터 최대한 많은
가능성을 생각해두고, 점차 줄여나가는 방식으로 기획을 진행했다.

**컨셉**

<b>`부품`</b>이라는 데이터를 중심으로 돌아가는 커뮤니티 기능을 가진 서비스이다.
부품을 모아 <b>`견적`</b>라는 또 다른 데이터를 만들 수 있다. 이 견적서는 부품의 최저가로
구성되게끔 최적화 할 수 있으다. 또한, 부품이나 견적서를 리뷰하거나 공유할 수 있다.
...
<details>
  ...
  동네를 타게팅하는 여러 플랫폼처럼, 집근처에서 조립해줄 사람을 구할 수 있다. 접근성이
...
<details>
  ...
  `견적서와 함께 글쓰기`에서 쓰여진 글에서 `이륙`을 누르면, 해당 견적서가 문제가 없고
...
기획 당시, 작성한 기능들 중 현재의 내가 구현이 가능한지 불가능한지 그 여부를 모르는 부분도 많았고,
...
능러닝커브가 생각보다 가팔랐고, 여기에 집중하기에는 시간이 부족했기 때문에, `Elasticsearch`가
...
<br />
이런식으로 이전에 개발해보지 않았던 기능을 미리 하나씩 구현해보면서, 기술적으로 구현이
가능한지 불가능한지를 파악했다. 대부분의 기능은 구현이 가능했고 이런 과정을 통해
굳이 필요없거나 과하게 기획 된 기능은 제외했다.
...
토글했을 때, 보통 네이티브앱에선 `뒤로가기`를 통해 메뉴를 닫을 수 있다. 가끔 웹에서 `뒤로가기`를 통해 메뉴를 닫는 경우도
...
모바일에선 모달이 아닌 전체 페이지 형태로 보여지니 뒤로가기를 통해 로그인 페이지로
...
스크래퍼 서버의 경우, 성능이 좋은 서버가 필요하기 때문에 상용 클라우드나 프리티어를 통해 해결할 수 없다.
...
대규모 데이터를 지속가능하게 수집하기 위해선 대상이 되는 상용 서버에 무리가 안 가게 그리고 지속적인 수집을 알아차리지 못 하게 해야했다.
...
등등 여러 변수가 생각났었는데, 이를 모두 한 프로그램에서 처리하려고 하니 너무 복잡해지고, 유지보수가 어려워질 것이라 생각했다.
...
프론트엔드는 `react`를 사용했다. 글을 쓰는 현재에는 `next.js`를 사용해봤기 때문에 추후에 `next.js`를 사용할 수도 있겠지만,
...
컴포넌트 단위로 개발한 이유는, 현재 프로젝트가 단 하나의 서비스이기 때문에 재사용 가능한 아토믹 컴포넌트나 디자인 시스템이 필요하기 보다는
...
<details>
  <summary>최저가 로직</summary>
    ...
    이 이상은 시간을 너무 많이 잡아먹는다. 대략 5초 이상 걸리기 때문에 이 범위를 넘어서지 않게끔 파라미터를 조절해야한다.
...
API의 갯수가 처음 계획했던 것보다 많아졌다. 그래서 `Swagger`를 통해 `API`를 문서화했다.
...
이번엔 서버를 한 번도 올리지 않고 구축 된 테스트 환경 내에서 모든 `API`를 작성했다. 웹개발을 하다보면 `Postman` 같은 툴을 사용하려고
