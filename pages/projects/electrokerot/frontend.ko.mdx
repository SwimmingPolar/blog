...
플랫폼이나 에이전시 일 경우, 디자인 시스템을 구축하는 것이 좋을 것 같다.
...
스토리북을 처음 쓰다보면 어떤 컴포넌트들이 의존하는 컨텍스트들이 있는데,
...
수정을 하다보면 `UI/UX`가 깨지는 경우가 종종 발생한다. 이런 경우를 캐치하기 위해 `Cypress`를 적용했다.
...
`react`사용 경험이 많이 없을 땐, 오히려 `redux`로 많은 걸 해결할 줄 알았는데 사용해보니 컨텍스트API를 사용하는 것이 훨씬 편하고 그렇게 해야하만 하는 경우가 많았다. 특히, 반응형에 대응할 때 많이 썼던 `useDeviceDetect`라는 훅을 위해 컨텍스트API를 사용했는데
간단하면서도 편리했다.
