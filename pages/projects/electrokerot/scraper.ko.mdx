...
스크래퍼를 설계할 때, 다음과 같은 목표를 가지고 설계했다.
...
실시간으로 데이터를 스크래핑하고 저장한다. 저장 된 데이터를 바탕으로 응답을 보낸다.
...
- 데이터를 실시간으로 스크래핑한다고 해도, 만약 온라인에 존재하는 데이터가 실시간이 아니라면?
  판매자가 상품의 가격을 업데이트하는 시간이 정해져있지 않기 때문에 실시간으로
...
클라우드 호스팅을 이용해 배포를 할 예정이기 때문에, 이렇다할 인프라 구조를 가지고 있을 필요가 없다.
...
`Updater`와 `Crawler`가 현재 상태에서 한 번에 처리할 수 있는 요청도 정해져있기 때문에,
